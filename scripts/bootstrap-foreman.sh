...
then
    ...
    sudo foreman-installer --foreman-admin-password=admin --puppet-server-max-active-instances=1 --puppet-server-jvm-min-heap-size=768M --puppet-server-jvm-max-heap-size=768M --puppet-server-implementation=puppetserver --puppet-autosign=true --puppet-autosign-entries='*.example.com' --puppet-hiera-config=/etc/puppetlabs/code/environments/hieradata/hiera.yaml
